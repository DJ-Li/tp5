...
{
    protected $base_model, $user_model;
    ...
    public function lists()
    {
        ...
        $size = 3;
        $list = $this->base_model->get_list('user', $tmp, $order, $p, $size);
        $data = [
            'status' => 200,
            'msg' => '获取成功',
            'data' => ['list' => $list['list']],
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            //检查登录昵称是否存在
            $name = $this->user_mode->check_name($data['user_login']);
            if (!$name){
            ...
            $data['user_pass'] = my_md5($data['user_pass']);
            if (empty($data['user_mobile']) || !check_tel($data['user_mobile'], 'sj')) {
                return json(['status' => AjaxCode::PARAM_VALID, 'msg' => '请填写正确的手机格式！']);
            }
            if (!empty($data['user_email'])) {
                if (!check_email($data['user_mobile'])) {
                    return json(['status' => AjaxCode::PARAM_VALID, 'msg' => '请填写正确的邮箱格式！']);
                }
            }
            $data['user_type'] = 1;
            //检查电话
            $tel = $this->user_mode->check_mobile($data['user_mobile']);
            if (!$tel){ //如果存在就修改用户类型
                $save = $this->base_model->edit_post('user',$data,$data['user_mobile']);
                if (!$save){
                    return json(['status' => AjaxCode::FAIL, 'msg' => '添加失败！']);
                }
                return json(['status' => AjaxCode::SUCCESS, 'msg' => '添加成功！']);
            }else{
                $add = $this->base_model->add_post('user',$data);
                if (!$add){
                    return json(['status' => AjaxCode::FAIL, 'msg' => '添加失败！']);
                }
                return json(['status' => AjaxCode::SUCCESS, 'msg' => '添加成功！']);
